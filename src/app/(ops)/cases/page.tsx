...
const validateFile = (file: File): { isValid: boolean; error?: string } => {
  const maxLimit = 5 * 1024 * 1024 * 1024;
...
// Only Category, a Case File, a Tooth Selection, and the Model Required
// choice are required to submit — every category sub-type field is optional.
...
export default function CasesPage() {
  ...
  // No default — the lab/ops user must actively pick Yes/No; see handleSubmit's guard.
  ...
  const handleSubmit = async () => {
    if (!hasAllRequiredCaseFields(category, subTypeData, notes, teeth, uploadedFile, modelRequired)) {
      toast.error("Please select a category, choose teeth, upload a file, and specify whether a model is required.");
      return;
    }
    const formData = new FormData();
    const caseData = {
  ...
  const handleBulkSubmit = async () => {
    ...
    if (bulkRows.some((row) => !hasAllRequiredCaseFields(row.category, row.subTypeData, row.notes, row.teeth, row.uploadedFile, row.modelRequired))) {
      toast.error("Complete category, teeth selection, file upload, and the Model Required choice for every case.");
      return;
    }
    const formData = new FormData();
        ...
        <div className="flex items-center justify-between flex-wrap gap-3">
          ...
          <div className="flex gap-2 items-center">
            ...
            <Button
              ...
              className="h-8 text-xs gap-1.5"
              onClick={() => {
                const headers = ["Case Name", "Case #", "Category", "Type / Restoration", "Teeth / Arch Selection", "Unit Count", "Numbering System", "Status", "Designer", "Due Date", "Created At"]
        ...
        <Card className="shadow-card border-border/50">
          <CardContent className="p-3 space-y-2.5">
            ...
            <div className="flex gap-1 flex-wrap pt-0.5">
              {statusFilters.map((s) => (
                <Button key={s} variant={statusFilter === s ? "default" : "outline"} size="sm" className="h-7 px-2.5 text-xs font-normal" onClick={() => setStatusFilter(s)}>{s}</Button>
              ))}
            </div>
          </CardContent>
        </Card>
                                  ...
                                  {c.status === "internal_qc" && (
                                    <div className="flex flex-wrap gap-1 items-center">
                                      ...
                                      <Button size="sm" variant="destructive" disabled={isMutating || !!pendingCaseAction}
                                        ...
                                        className="h-7 text-[10px] px-2 py-0.5 font-semibold  uppercase tracking-wider bg-red-600 hover:bg-red-700 shadow-sm">✗ Reject</Button>
                                      <Button size="sm" disabled={isMutating || !!pendingCaseAction}
