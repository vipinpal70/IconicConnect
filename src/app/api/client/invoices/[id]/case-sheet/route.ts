...
function extractCaseRow(
  ...
  let subType = '—', selection = '—', units = 0, unitPrice = 0, isArchBased = false
  ...
  if (cat === 'crown & bridge' || cat === 'crown & bridges') {
    ...
    selection = [...impTeeth.map((t) => `Imp:#${t}`), ...cbTeeth.map((t) => `CB:#${t}`)].join(', ') || '—'
    ...
    units = archCount; unitPrice = getPrice('Appliances', subType); isArchBased = true
  } else if (cat === 'denture' || cat === 'dentures') {
    subType = String(d.sub_category || d.subCategory || d.caseType1 || 'Full Denture')
    selection = String(d.arch || d.caseType2 || 'Upper')
    units = archCount; unitPrice = getPrice('Dentures', subType); isArchBased = true
  } else if (cat === 'cosmetics' || cat === 'cosmetic') {
    subType = String(d.sub_category || d.subCategory || d.caseType || d.caseType1 || 'Veneers')
    selection = String(d.arch || d.caseType2 || 'Upper')
    units = archCount; unitPrice = getPrice('Cosmetics', subType); isArchBased = true
  }

  const unitsLabel = isArchBased ? `${units} arch${units !== 1 ? 'es' : ''}` : `${units} unit${units !== 1 ? 's' : ''}`
  const caseTotal = parseFloat((units * unitPrice + modelPrice).toFixed(2))

  return { caseNumber: c.caseNumber ?? '—', category: c.category ?? '—', subType, selection, unitsLabel, modelRequired: modelRequired ? 'Yes' : 'No', unitPrice, caseTotal }
}
  ...
  try {
    ...
    if (caseIds.length > 0) {
      const [caseRows, catalogItems, clientPriceRows] = await Promise.all([
        ...
        db.select({ category: serviceCatalog.category, subCategory: serviceCatalog.subCategory, defaultPrice: serviceCatalog.defaultPrice })
          .from(serviceCatalog).where(eq(serviceCatalog.isActive, true)),
        db.select({ category: serviceCatalog.category, subCategory: serviceCatalog.subCategory, price: clientPriceList.price })
      ...
      for (const c of caseRows) {
        const r = extractCaseRow({ caseNumber: c.caseNumber, category: c.category, subTypeData: c.subTypeData, clientId: c.clientId }, getPrice)
      ...
      csvLines.push(row('Note: Individual case data not available for this invoice.'))
      csvLines.push('')
    ...
    return new NextResponse(csv, {
      ...
      headers: {
        ...
        'Content-Disposition': `attachment; filename="${inv.invoiceNumber}-case-sheet.csv"`,
        'Cache-Control': 'no-store',
      },
    })
  } catch (err) {
    console.error('[client/invoices/[id]/case-sheet GET]', err)
