 ...
 * field. This mirrors [id]/files/route.ts's GET (same auth pattern), minus
 * the POST — there's no separate "attach a reference image after the fact"
 * flow yet.
 */
export async function GET(
  ...
  try {
    ...
    const images = await db.select().from(caseReferenceFiles).where(eq(caseReferenceFiles.caseId, id));
