...
type CasePayload = {
  ...
  dueDate?: string;
  // 3Shape XML Import only: when true, an entry whose zip name + tooth
  // selection matches an existing ACTIVE case for this client is silently
  // skipped instead of 409-ing the whole request.
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (profile.role === 'client' || profile.role === 'subuser') {
      ...
      // case by zip name + tooth-selection overlap. ---
    ...
    for (let i = 0; i < casesArray.length; i++) {
      ...
      const modelRequired = (caseData.subTypeData as { modelRequired?: unknown } | undefined)?.modelRequired;
      if (modelRequired !== 'yes' && modelRequired !== 'no') {
        return NextResponse.json(
