...
export default function ClientPreferencesPage() {
  ...
  const duplicateForm = async (form: PreferenceFormRecord) => {
    ...
    try {
      ...
      if (!res.ok) {
        const errorBody = await res.json().catch(() => null)
        throw new Error(errorBody?.error || "Failed to duplicate preference form")
      }
      ...
      console.error(error)
      toast.error(error instanceof Error ? error.message : "Failed to duplicate preference form")
    } finally {
    ...
    <div className="mx-auto max-w-5xl space-y-4 animate-fade-in">
              ...
              <table className="w-full text-xs border border-border/40 rounded-lg overflow-hidden">
                <thead>
                  <tr className="bg-muted/40 border-b border-border/40">
                    <th className="text-left px-3 py-2 font-semibold text-muted-foreground">Form Name</th>
                    <th className="text-left px-3 py-2 font-semibold text-muted-foreground">Occlusion</th>
                    <th className="text-left px-3 py-2 font-semibold text-muted-foreground">Anatomy</th>
                    <th className="text-left px-3 py-2 font-semibold text-muted-foreground">Pontic Type</th>
                    <th className="text-left px-3 py-2 font-semibold text-muted-foreground">Created</th>
                ...
                <tbody className="divide-y divide-border/30">
                  {forms.map((form) => (
                    <tr key={form.id} className="hover:bg-muted/10 transition-colors">
                      <td className="px-3 py-2 font-medium text-foreground">{form.formName}</td>
                      <td className="px-3 py-2 text-muted-foreground">{form.payload.occlusion.defaultValues || "-"}</td>
                      <td className="px-3 py-2 text-muted-foreground">{form.payload.anatomy.option || "-"}</td>
                      <td className="px-3 py-2 text-muted-foreground">{form.payload.ponticType.option || "-"}</td>
                      <td className="px-3 py-2 text-muted-foreground">{new Date(form.createdAt).toLocaleDateString()}</td>
                      <td className="px-3 py-2 text-muted-foreground">{new Date(form.updatedAt).toLocaleDateString()}</td>
                      <td className="px-3 py-2">
                        <div className="flex items-center justify-end gap-1">
                          <Button variant="ghost" size="icon" className="h-7 w-7" title="Edit" onClick={() => editForm(form)}>
                          ...
                          <Button variant="ghost" size="icon" className="h-7 w-7 text-destructive hover:bg-destructive/10" title="Delete" onClick={() => deleteForm(form.id)}>
      ...
      <Dialog open={modalOpen} onOpenChange={(open) => (open ? setModalOpen(true) : closeModal())}>
        <DialogContent className="max-w-2xl max-h-[85vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle className="text-sm">{STEP_LABELS[formStep]}</DialogTitle>
            <p className="text-[11px] text-muted-foreground">
              {editingId ? "Editing preference form" : "Add a new preference form"} — Step {formStep} of 5
            </p>
          </DialogHeader>
          ...
          <div className="space-y-3.5 animate-fade-in">
              ...
              <div className="space-y-3.5 animate-fade-in">
                <Section label="Gingiva Levels (mm)">
                  <div className="grid gap-3 sm:grid-cols-2">
                    <div className="space-y-1.5">
                      <p className="text-[10px] font-bold uppercase tracking-wide text-muted-foreground">Anterior</p>
                      <div className="grid grid-cols-3 gap-1.5">
                        <NumberField
                          label="Buccal"
                          value={draft.payload.gingivaLevels?.anteriorBuccal || ""}
                          onChange={(v) => updatePayload("gingivaLevels", { ...draft.payload.gingivaLevels, anteriorBuccal: v } as PreferenceFormPayload["gingivaLevels"])}
                        />
                        <NumberField
                          label="Lingual"
                          value={draft.payload.gingivaLevels?.anteriorLingual || ""}
                          onChange={(v) => updatePayload("gingivaLevels", { ...draft.payload.gingivaLevels, anteriorLingual: v } as PreferenceFormPayload["gingivaLevels"])}
                        />
                        <NumberField
                          label="Mesial & Distal"
                          value={draft.payload.gingivaLevels?.anteriorMesialDistal || ""}
                          onChange={(v) => updatePayload("gingivaLevels", { ...draft.payload.gingivaLevels, anteriorMesialDistal: v } as PreferenceFormPayload["gingivaLevels"])}
                        />
                      </div>
                    </div>
                    <div className="space-y-1.5">
                      <p className="text-[10px] font-bold uppercase tracking-wide text-muted-foreground">Posterior</p>
                      <div className="grid grid-cols-3 gap-1.5">
                        <NumberField
                          label="Buccal"
                          value={draft.payload.gingivaLevels?.posteriorBuccal || ""}
                          onChange={(v) => updatePayload("gingivaLevels", { ...draft.payload.gingivaLevels, posteriorBuccal: v } as PreferenceFormPayload["gingivaLevels"])}
                        />
                        <NumberField
                          label="Lingual"
                          value={draft.payload.gingivaLevels?.posteriorLingual || ""}
                          onChange={(v) => updatePayload("gingivaLevels", { ...draft.payload.gingivaLevels, posteriorLingual: v } as PreferenceFormPayload["gingivaLevels"])}
                        />
                        <NumberField
                          label="Mesial & Distal"
                          value={draft.payload.gingivaLevels?.posteriorMesialDistal || ""}
                          onChange={(v) => updatePayload("gingivaLevels", { ...draft.payload.gingivaLevels, posteriorMesialDistal: v } as PreferenceFormPayload["gingivaLevels"])}
                        />
                      </div>
                    </div>
                  </div>
                  <Input
                    ...
                    onChange={(e) => updatePayload("gingivaLevels", { ...draft.payload.gingivaLevels, comments: e.target.value } as PreferenceFormPayload["gingivaLevels"])}
                    placeholder="Comments"
                  />
                </Section>
                ...
                <Section label="Distance to Antagonist">
                  <ChoiceRow
                    ...
                    onChange={(option) => updatePayload("distanceToAntagonist", { ...draft.payload.distanceToAntagonist, option, comments: draft.payload.distanceToAntagonist?.comments || "" })}
                  />
                  <Input
                    className="h-8 text-xs mt-1"
                    value={draft.payload.distanceToAntagonist?.comments || ""}
                    onChange={(e) => updatePayload("distanceToAntagonist", { ...draft.payload.distanceToAntagonist, option: draft.payload.distanceToAntagonist?.option || "", comments: e.target.value })}
                    placeholder="Comments"
                  />
                </Section>
                ...
                <Section label="Identification Dots">
                  <ChoiceRow
                    ...
                    options={yesNoOptions}
                    onChange={(option) => updatePayload("identificationDots", { ...draft.payload.identificationDots, option, comments: draft.payload.identificationDots?.comments || "" })}
                  />
                  <Input
                    className="h-8 text-xs mt-1"
                    value={draft.payload.identificationDots?.comments || ""}
                    onChange={(e) => updatePayload("identificationDots", { ...draft.payload.identificationDots, option: draft.payload.identificationDots?.option || "", comments: e.target.value })}
                    placeholder="Comments"
                  />
                </Section>
                ...
                <Section label="Internal Retention Groove">
                  <ChoiceRow
                    ...
                    options={yesNoOptions}
                    onChange={(option) => updatePayload("internalRetentionGroove", { ...draft.payload.internalRetentionGroove, option, comments: draft.payload.internalRetentionGroove?.comments || "" })}
                  />
                  <Input
                    className="h-8 text-xs mt-1"
                    value={draft.payload.internalRetentionGroove?.comments || ""}
                    onChange={(e) => updatePayload("internalRetentionGroove", { ...draft.payload.internalRetentionGroove, option: draft.payload.internalRetentionGroove?.option || "", comments: e.target.value })}
                    placeholder="Comments"
                  />
                </Section>
                ...
                <Section label="Taper Angle">
                  <ChoiceRow
                    ...
                    onChange={(option) => updatePayload("taperAngle", { ...draft.payload.taperAngle, option, comments: draft.payload.taperAngle?.comments || "" })}
                  />
                  <Input
                    className="h-8 text-xs mt-1"
                    value={draft.payload.taperAngle?.comments || ""}
                    onChange={(e) => updatePayload("taperAngle", { ...draft.payload.taperAngle, option: draft.payload.taperAngle?.option || "", comments: e.target.value })}
                    placeholder="Comments"
                  />
                </Section>
                ...
                <Section label="Emergence Profile">
                  <ChoiceRow
                    ...
                    onChange={(option) => updatePayload("emergenceProfile", { ...draft.payload.emergenceProfile, option, comments: draft.payload.emergenceProfile?.comments || "" })}
                  />
                  <Input
                    className="h-8 text-xs mt-1"
                    value={draft.payload.emergenceProfile?.comments || ""}
                    onChange={(e) => updatePayload("emergenceProfile", { ...draft.payload.emergenceProfile, option: draft.payload.emergenceProfile?.option || "", comments: e.target.value })}
                    placeholder="Comments"
                  />
                </Section>
                ...
                <Section label="Screw-retained Crown (Hole Size)">
                  <ChoiceRow
                    ...
                    onChange={(option) => updatePayload("screwRetainedCrown", { ...draft.payload.screwRetainedCrown, option, comments: draft.payload.screwRetainedCrown?.comments || "" })}
                  />
                  <Input
                    className="h-8 text-xs mt-1"
                    value={draft.payload.screwRetainedCrown?.comments || ""}
                    onChange={(e) => updatePayload("screwRetainedCrown", { ...draft.payload.screwRetainedCrown, option: draft.payload.screwRetainedCrown?.option || "", comments: e.target.value })}
                    placeholder="Comments"
                  />
                </Section>
              </div>
            ...
            <div className="pt-2 flex items-center gap-2">
              <Button
                ...
                disabled={formStep === 1}
                onClick={() => setFormStep((step) => (step > 1 ? ((step - 1) as FormStep) : step))}
              >
                Previous
              </Button>
              ...
              {formStep < 5 ? (
                <Button
                  ...
                  onClick={() => setFormStep((step) => (step < 5 ? ((step + 1) as FormStep) : step))}
                >
                  Next
                </Button>
              ) : (
